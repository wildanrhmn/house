...
Landing: [http://localhost:3000](http://localhost:3000). Desk: [http://localhost:3000/desk](http://localhost:3000/desk). Connect a Shannon wallet, then Quote both sides once per window. The book lights your two rows, a minted set shows up as you hold it, Flatten merges balanced sets back to collateral and sells any leftover, and Redeem settled pulls payouts after resolve. `/desk?watch=0x...` follows any wallet read-only, which is how to watch the Node quoter work.
